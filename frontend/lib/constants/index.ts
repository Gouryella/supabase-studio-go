...
const env = (key: string) => (runtimeEnv as Record<string, string | undefined>)[key] ?? process.env[key]

export const IS_PLATFORM = env('NEXT_PUBLIC_IS_PLATFORM') === 'true'
...
export const API_URL = (() => {
  ...
  if (IS_PLATFORM) return env('NEXT_PUBLIC_API_URL')!
  ...
  if (!!env('NEXT_PUBLIC_SITE_URL')) return `${env('NEXT_PUBLIC_SITE_URL')}/api`
  return '/api'
})()
...
export const BASE_PATH = env('NEXT_PUBLIC_BASE_PATH') ?? ''
...
export const STRIPE_PUBLIC_KEY =
  env('NEXT_PUBLIC_STRIPE_PUBLIC_KEY') || 'pk_test_XVwg5IZH3I9Gti98hZw6KRzd00v5858heG'

export const POSTHOG_URL =
  env('NEXT_PUBLIC_ENVIRONMENT') === 'staging' || env('NEXT_PUBLIC_ENVIRONMENT') === 'local'
    ? 'https://ph.supabase.green'
    : 'https://ph.supabase.com'

export const USAGE_APPROACHING_THRESHOLD = 0.75

export const DOCS_URL = env('NEXT_PUBLIC_DOCS_URL') || 'https://supabase.com/docs'
