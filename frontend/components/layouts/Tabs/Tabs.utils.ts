...
export function useSqlEditorTabsCleanup() {
  ...
  return useCallback(({ snippets }: { snippets: { id: string; type: string; name: string }[] }) => {
    ...
    // Defensive: upstream data can occasionally contain nulls (e.g. malformed API responses).
    const sanitizedSnippets = (snippets ?? []).filter(
      (snippet): snippet is { id: string; type: string; name: string } =>
        Boolean(snippet) && typeof snippet === 'object'
    )
    ...
    openSqlTabs.forEach((tab) => {
      const snippet = sanitizedSnippets.find((x) => tab.metadata?.sqlId === x.id)
