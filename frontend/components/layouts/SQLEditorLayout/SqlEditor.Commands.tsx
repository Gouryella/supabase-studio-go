...
function RunSnippetPage() {
  ...
  const snippets = useMemo(
    () =>
      (snippetPages?.pages.flatMap((page) => page.contents) ?? []).filter(
        (snippet): snippet is SqlSnippet => Boolean(snippet)
      ),
    [snippetPages?.pages]
  )
...
function snippetValue(snippet: SqlSnippet | null | undefined) {
  if (!snippet || snippet.type !== 'sql') return ''
