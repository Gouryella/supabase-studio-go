...
import { decodeStorageObjectPath } from '../StorageObjectKeyCodec.utils'
...
export const formatFolderItems = (items: StorageObject[] = [], prefix?: string): StorageItem[] => {
  const formattedItems =
    (items ?? [])
      .map((item) => {
        const decodedName = decodeStorageObjectPath(item.name)
        if (decodedName === EMPTY_FOLDER_PLACEHOLDER_FILE_NAME) return null
