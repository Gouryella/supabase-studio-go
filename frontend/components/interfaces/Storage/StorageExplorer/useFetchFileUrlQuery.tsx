...
import { encodeStorageObjectPath } from '../StorageObjectKeyCodec.utils'
...
export const fetchFileUrl = async (
  ...
  const encodedPathToFile = encodeStorageObjectPath(pathToFile).replace(/^\/+/, '')

  if (isBucketPublic) {
    const data = await getPublicUrlForBucketObject({
      projectRef: projectRef,
      bucketId: bucketId,
      path: encodedPathToFile,
    })
