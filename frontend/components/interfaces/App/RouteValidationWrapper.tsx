...
export const RouteValidationWrapper = ({ children }: PropsWithChildren<{}>) => {
  ...
  const DEFAULT_HOME = IS_PLATFORM
    ? !!lastVisitedOrganization
      ...
      : '/organizations'
    : '/project/default'
  const projectRefFromQuery =
    typeof router.query.projectRef === 'string' ? router.query.projectRef : undefined
  const SELF_HOSTED_HOME = projectRefFromQuery ? `/project/${projectRefFromQuery}` : '/project/default'
  const isSelfHostedOrgRoute =
    !IS_PLATFORM &&
  ...
  useEffect(() => {
    if (!router.isReady || !isSelfHostedOrgRoute) return
    router.replace(SELF_HOSTED_HOME)
  }, [isSelfHostedOrgRoute, router, SELF_HOSTED_HOME])
