...
export const AIAssistant = ({ className }: AIAssistantProps) => {
  ...
  const defaultModel = useMemo(() => {
    if (check?.defaultModel && modelOptions.includes(check.defaultModel)) {
      return check.defaultModel
    }
    return isPaidPlan ? 'gpt-5' : 'gpt-5-mini'
  }, [check?.defaultModel, isPaidPlan, modelOptions])

  const selectedModel = useMemo(() => {
    let model = snap.model ?? defaultModel

    if (!modelOptions.includes(model)) {
      model = modelOptions[0] ?? defaultModel
    }

    if (IS_PLATFORM && !isPaidPlan && model === 'gpt-5' && modelOptions.includes('gpt-5-mini')) {
      model = 'gpt-5-mini'
    }

    return model
  }, [defaultModel, isPaidPlan, modelOptions, snap.model])

  useEffect(() => {
    if (snap.model !== selectedModel) {
      setModel(selectedModel)
    }
  }, [selectedModel, setModel, snap.model])
      ...
      <div className={cn('flex flex-col h-full w-full md:h-full max-h-[100dvh]', className)}>
        ...
        <div className="px-3 pb-3 z-20 relative">
          ...
          {isSuccess && !isApiKeySet && (
            <Admonition
              ...
              description={
                <Markdown
                  content={
                    'Add your `OPENAI_API_KEY` to your environment variables to use the AI Assistant. You can also set `OPENAI_API_URL` to use a custom API base URL and `OPENAI_MODELS` to control the model list (e.g. `["gpt-4o-mini","gpt-4o"]`).'
                  }
                />
              }
            />
          ...
          <AssistantChatForm
            ...
            onSelectModel={(model) => state.setModel(model)}
          />
        </div>
      </div>
    </ErrorBoundary>
