...
export const ClientSideExceptionHandler = ({
  ...
  const handleClearStorage = () => {
    try {
      ...
      // Clear any persisted AI assistant state which lives in IndexedDB.
      // Corrupted message parts can cause client-side render crashes (e.g. `part.type` on `null`).
      indexedDB?.deleteDatabase?.('ai-assistant-db')
    } catch (e) {
      // ignore
    }
    ...
    <>
      <div className="flex flex-col gap-y-1 text-left py-2 w-full">
        <div className="flex items-center justify-between mb-3">
          ...
          <CopyButton
            type="outline"
            text={[message, urlMessage].filter(Boolean).join('\n\n')}
        ...
        {urlMessage && (
          <details className="mt-2">
            <summary className={cn(InlineLinkClassName, 'text-sm cursor-pointer')}>
              Show stack trace
            </summary>
            <pre className="mt-2 text-xs whitespace-pre-wrap break-words text-foreground-light">
              {urlMessage}
            </pre>
          </details>
        )}
