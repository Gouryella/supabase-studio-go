...
export type AssistantModel = string
...
function sanitizeForCloning(obj: any): any {
  ...
  try {
    ...
    console.warn('Failed to sanitize AI assistant state for cloning:', error)
    return null
  }
}
...
function sanitizeMessageParts(parts: unknown): MessagePart[] {
  if (!Array.isArray(parts)) return []

  const sanitized: MessagePart[] = []
  for (const part of parts) {
    if (!isObjectWithType(part)) continue

    const cloned = sanitizeForCloning(part)
    if (!isObjectWithType(cloned)) continue

    sanitized.push(cloned as MessagePart)
  }

  return sanitized
}

function sanitizeMessages(messages: unknown): AssistantMessageType[] {
  if (!Array.isArray(messages)) return []

  const sanitized: AssistantMessageType[] = []

  for (const message of messages) {
    if (!message || typeof message !== 'object') continue

    const cloned = sanitizeForCloning(message)
    if (!cloned || typeof cloned !== 'object') continue

    ;(cloned as { parts?: unknown }).parts = sanitizeMessageParts((cloned as { parts?: unknown }).parts)

    sanitized.push(cloned as AssistantMessageType)
  }

  return sanitized
}

function sanitizeChats(chats: unknown): Record<string, ChatSession> {
  if (!chats || typeof chats !== 'object') return {}

  const sanitized: Record<string, ChatSession> = {}

  for (const [chatId, rawChat] of Object.entries(chats as Record<string, unknown>)) {
    if (!rawChat || typeof rawChat !== 'object') continue

    const chat = rawChat as Partial<ChatSession> & { messages?: unknown }

    sanitized[chatId] = {
      id: typeof chat.id === 'string' ? chat.id : chatId,
      name: typeof chat.name === 'string' ? chat.name : 'New chat',
...
async function loadFromIndexedDB(projectRef: string): Promise<StoredAiAssistantState | null> {
  try {
    ...
    if (persistedState) {
      persistedState.chats = sanitizeChats(persistedState.chats)
  ...
  try {
    ...
    if (parsedFromLocalStorage && typeof parsedFromLocalStorage.chats === 'object') {
      migratedState = {
        ...
        chats: sanitizeChats(parsedFromLocalStorage.chats),
...
function createChatInstance(
  ...
  return new Chat<MessageType>({
    id: options.id,
    messages: sanitizeMessages(options.initialMessages),
...
export const createAiAssistantState = (): AiAssistantState => {
  ...
  const state: AiAssistantState = proxy({
    ...
    loadPersistedState: (persistedState: StoredAiAssistantState) => {
      state.chats = sanitizeChats(persistedState.chats)
