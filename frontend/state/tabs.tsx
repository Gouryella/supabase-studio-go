...
function getSavedRecentItems(ref: string): RecentItem[] {
  ...
  try {
    const parsed = JSON.parse(stored ?? '{"items": []}') as { items?: unknown }
    const items = Array.isArray(parsed.items) ? parsed.items : []

    return items.filter((item): item is RecentItem => {
...
function getSavedTabs(ref: string) {
  ...
  try {
    ...
    // Local storage can end up containing `null` values in arrays/maps; sanitize
    // these eagerly to avoid client-side crashes on boot.
    const sanitizedTabsMap: Record<string, Tab> = {}
    for (const [key, value] of Object.entries(parsed.tabsMap)) {
      if (!value || typeof value !== 'object') continue
      const candidate = value as Tab
      if (typeof candidate.id !== 'string' || typeof candidate.type !== 'string') continue
      sanitizedTabsMap[key] = candidate
    }

    const sanitizedOpenTabs = parsed.openTabs
      .filter((tabId): tabId is string => typeof tabId === 'string')
      .filter((tabId) => sanitizedTabsMap[tabId] !== undefined)

    const sanitizedActiveTab =
      typeof parsed.activeTab === 'string' && sanitizedTabsMap[parsed.activeTab] !== undefined
        ? parsed.activeTab
        : null

    const sanitizedPreviewTabId =
      typeof parsed.previewTabId === 'string' &&
      sanitizedTabsMap[parsed.previewTabId] !== undefined
        ? parsed.previewTabId
        : undefined
    ...
    return {
      ...
      openTabs: sanitizedOpenTabs,
      tabsMap: sanitizedTabsMap,
      previewTabId: sanitizedPreviewTabId,
    }
  } catch (error) {
    return DEFAULT_TABS_STATE
  }
