...
import { validObjectKeyRegex } from 'components/interfaces/Storage/CreateBucketModal.utils'
import {
  STORAGE_BUCKET_SORT,
  STORAGE_ROW_STATUS,
  STORAGE_ROW_TYPES,
...
import {
  ...
  encodeStorageObjectSegment,
} from 'components/interfaces/Storage/StorageObjectKeyCodec.utils'
...
function createStorageExplorerState({
  ...
  const state = proxy({
    ...
    validateFolderName: (name: string) => {
      ...
      const encodedName = encodeStorageObjectSegment(name)
      if (!validObjectKeyRegex.test(encodedName)) {
        return 'Folder name contains an invalid special character'
      }
    ...
    addNewFolder: async ({
      ...
      const encodedEmptyPlaceholderFile = `${encodeStorageObjectSegment(
        formattedName
      )}/${EMPTY_FOLDER_PLACEHOLDER_FILE_NAME}`
      const pathToFolder = encodeStorageObjectPath(
        state.openedFolders
          .slice(0, columnIndex)
          .map((folder) => folder.name)
          .join('/')
      ...
      const formattedPathToEmptyPlaceholderFile =
        pathToFolder.length > 0
          ? `${pathToFolder}/${encodedEmptyPlaceholderFile}`
          : encodedEmptyPlaceholderFile
    ...
    fetchFolderContents: async ({
      ...
      const prefix = encodeStorageObjectPath(
        state.openedFolders
          .slice(0, index + 1)
          .map((folder) => folder.name)
          .join('/')
    ...
    fetchMoreFolderContents: async ({
      ...
      const encodedPrefix = encodeStorageObjectPath(prefix)
      const options = {
        limit: LIMIT,
      ...
      try {
        ...
        // Add items to column
        const formattedItems = formatFolderItems(data, encodedPrefix)
        state.columns = state.columns.map((col, idx) => {
          if (idx === index) {
            return {
              ...col,
    ...
    fetchFoldersByPath: async ({
      ...
      const foldersItems = await Promise.all(
        pathsWithEmptyPrefix.map(async (path, idx) => {
          const prefix = encodeStorageObjectPath(paths.slice(0, idx).join('/'))
          const options = {
            limit: LIMIT,
            offset: OFFSET,
            search: searchString,
    ...
    validateParentFolderEmpty: async (parentFolderPrefix: string) => {
      try {
        const encodedParentFolderPrefix = encodeStorageObjectPath(parentFolderPrefix)
        const data = await listBucketObjects({
          projectRef: state.projectRef,
          bucketId: state.selectedBucket.id,
          path: encodedParentFolderPrefix,
          options: {
            limit: LIMIT,
            offset: OFFSET,
            sortBy: { column: state.sortBy, order: state.sortByOrder },
        ...
        if (data.length === 0) {
          const prefixToPlaceholder = `${encodedParentFolderPrefix}/${EMPTY_FOLDER_PLACEHOLDER_FILE_NAME}`
          const client = await createProjectSupabaseClient(state.projectRef, clientEndpoint)
          await client.storage
            .from(state.selectedBucket.name)
      ...
      try {
        ...
        if (files.length === 0) {
          ...
          await deleteBucketPrefix({
            ...
            bucketId: state.selectedBucket.id,
            prefix: folder.path ?? encodeStorageObjectPath(folder.name),
          })
        } else {
    ...
    renameFolder: async ({
      ...
      try {
        ...
        let progress = 0
        let hasErrors = false
        const encodedNewName = encodeStorageObjectSegment(newName)
      ...
      try {
        ...
        const promises = files.map((file) => {
          ...
          return () => {
            ...
            >(async (resolve) => {
              try {
                const encodedFilePath = `${file.prefix}/${file.name}`
                ...
                resolve({
                  name: decodeStorageObjectPath(file.name),
                  prefix: decodeStorageObjectPath(file.prefix),
    ...
    getAllItemsAlongFolder: async (folder: {
      ...
      if (prefix === undefined) {
        ...
        formattedPathToFolder =
          pathToFolder.length > 0 ? `${pathToFolder}/${encodeStorageObjectSegment(name)}` : name
        if (pathToFolder.length === 0) {
          formattedPathToFolder = encodeStorageObjectPath(name)
        }
      } else {
    ...
    uploadFiles: async ({
      ...
      let numberOfFilesAutoSanitized = 0

      const pathToFile = encodeStorageObjectPath(
        state.openedFolders
      ...
      const promises = formattedFilesToUpload.map((file, index) => {
        ...
        const duplicateSanitizedFileName = !isWithinFolder
          ? state.sanitizeNameForDuplicateInColumn({ name: file.name, autofix }) ?? file.name
          : file.name
        const unsanitizedFormattedFileName =
          has(file, ['path']) && isWithinFolder ? file.path : duplicateSanitizedFileName
        const displayFileName = (unsanitizedFormattedFileName ?? 'unknown').replaceAll(
          /\u{202F}/gu,
          ' '
        )
        const formattedFileName = encodeStorageObjectPath(displayFileName)
        if (formattedFileName !== displayFileName) {
          numberOfFilesAutoSanitized += 1
        }
        ...
        if (isWithinFolder) {
          const topLevelFolder = displayFileName.split('/')[0] || ''
      ...
      if (numberOfFilesAutoSanitized > 0) {
        toast(
          `Adjusted ${numberOfFilesAutoSanitized} file name${
            numberOfFilesAutoSanitized > 1 ? 's' : ''
          } to storage-safe keys`
        )
      }
    ...
    moveFiles: async (newPathToFile: string) => {
      ...
      await Promise.all(
        state.selectedItemsToMove.map(async (item) => {
          ...
          const encodedItemName = encodeStorageObjectSegment(item.name)
          const encodedNewPathToFile = encodeStorageObjectPath(formattedNewPathToFile)

          const fromPath =
            pathToFile.length > 0 ? `${pathToFile}/${encodedItemName}` : encodedItemName
          const toPath =
            newPathToFile.length > 0
              ? `${encodedNewPathToFile}/${encodedItemName}`
              : encodedItemName
    ...
    deleteFiles: async ({
      ...
      const prefixes = !files.some((f) => f.prefix)
        ? files.map((file) => {
            ...
            return pathToFile.length > 0 ? `${pathToFile}/${encodedName}` : encodedName
          })
        : files.map((file) => encodeStorageObjectPath(`${file.prefix}/${file.name}`))
    ...
    renameFile: async (file: StorageItem, newName: string, columnIndex: number) => {
      ...
      if (originalName === newName || newName.length === 0) {
        ...
        const encodedPathToFile = encodeStorageObjectPath(pathToFile)

        const encodedOriginalName = encodeStorageObjectSegment(originalName)
        const encodedNewName = encodeStorageObjectSegment(newName)
        const fromPath =
          encodedPathToFile.length > 0
            ? `${encodedPathToFile}/${encodedOriginalName}`
            : encodedOriginalName
        const toPath =
          encodedPathToFile.length > 0 ? `${encodedPathToFile}/${encodedNewName}` : encodedNewName
