...
export function usePHFlag<T = string | boolean>(name: string) {
  ...
  // On self-hosted / local environments, PostHog may be unavailable (no `/telemetry/feature-flags`
  // endpoint). In that case, treat flags as disabled (`false`) so the app can still render.
  if (isObjectEmpty(store)) return IS_PLATFORM ? undefined : (false as unknown as T)
